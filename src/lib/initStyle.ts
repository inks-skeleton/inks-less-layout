...
class CreateInitStyle {
  ...
  constructor(options: LSConfig) {
    ...
    const mobileFitRangeMax =
      mobileFitRange.length >= 1 ? mobileFitRange[1] : 0;
    const pcFontSizeCss = pcFontSize ? `font-size:${pcFontSize}px;` : "";
    ...
    if (ffMixin)
      webPage.fontFamily = "@fonts_css_" + webPage.fontFamily + "();";
